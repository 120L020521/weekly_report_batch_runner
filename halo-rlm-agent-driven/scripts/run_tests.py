...
def test_agent_cli() -> None:
    ...
    weekly_prompt = root / "weekly_prompt.txt"
    built_weekly = agent(
        "build-prompt", "--output", weekly_prompt, "--task-json", task,
        "--adapter", "weekly-report",
    )
    weekly_text = weekly_prompt.read_text(encoding="utf-8")
    check(
        built_weekly.returncode == 0
        and "weekly_runner.py" in weekly_text
        and "workspace_bench_tools.ts" not in weekly_text,
        "adapter selects weekly-report editable surfaces",
    )
    file_prompt = root / "file_prompt.txt"
    built_file = agent(
        "build-prompt", "--output", file_prompt, "--task-json", task,
        "--adapter", "file-organization",
    )
    file_text = file_prompt.read_text(encoding="utf-8")
    check(
        built_file.returncode == 0
        and "setup_device.py" in file_text
        and "weekly_runner.py" not in file_text,
        "adapter selects file-organization editable surfaces",
    )
    weekly_report = valid_report(change_count=1)
    weekly_report["proposed_changes"][0]["target"] = "weekly_runner.py"
    write_json(report, weekly_report)
    check(
        agent("validate-report", report, "--adapter", "weekly-report").returncode == 0,
        "report validation uses the same adapter surface policy",
    )
