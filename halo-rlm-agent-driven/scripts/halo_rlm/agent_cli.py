...
ADAPTER_EDITABLE_SURFACES: dict[str, tuple[str, ...]] = {
    "file-organization": (
        "xiaoyi-auto-continue/SKILL.md",
        "run_test.py",
        "task_executor.py",
        "setup_device.py",
    ),
    "weekly-report": (
        "xiaoyi-weekly-report/SKILL.md",
        "weekly_runner.py",
        "task_executor.py",
        "batch_execute_tools.py",
    ),
    "workspacebench": tuple(DEFAULT_EDITABLE_SURFACES),
    "workspace-bench": tuple(DEFAULT_EDITABLE_SURFACES),
}


def _resolve_surfaces(args: argparse.Namespace) -> list[str]:
    """Resolve one allowlist; an explicit --surface always wins."""
    if args.surface:
        return list(args.surface)
    adapter = (getattr(args, "adapter", None) or "workspacebench").strip().lower()
    return list(ADAPTER_EDITABLE_SURFACES.get(adapter, DEFAULT_EDITABLE_SURFACES))
...
def _build_prompt(args: argparse.Namespace) -> dict[str, Any]:
    ...
    prompt = build_halo_prompt(
        ...
        surface_filenames=_resolve_surfaces(args),
...
def _validate_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    surfaces = _resolve_surfaces(args)
...
def _parser() -> argparse.ArgumentParser:
    ...
    prompt.add_argument(
        "--adapter",
        default="workspacebench",
        help="Dataset adapter used to select the default editable-surface allowlist",
    )
