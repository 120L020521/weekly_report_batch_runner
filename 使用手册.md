...
```text
<agent_workspace>/xiaoyi_file_runs/run_<YYYYMMDD>/
├── runner_batch.json
└── FileOrganization_0_001/
...
& python -B "<skill_root>\run_test.py" `
  ...
  --config "<config.json>" `
  --date "20260818" `
  --output-base "<agent_workspace>\xiaoyi_file_runs"
```
...
& python -B "<skill_root>\scripts\halo_workflow.py" finalize-batch `
  --agent-queue "<xiaoyi_halo>\halo_agent_queue.json"
```
