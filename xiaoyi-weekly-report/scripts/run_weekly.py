#!/usr/bin/env python3
"""Launch the bundled XiaoYi weekly-report runner against external data."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[weekly-skill] data_root={data_root or '<separate paths>'}", flush=True)
    print(f"[weekly-skill] runtime_root={RUNTIME_ROOT}", flush=True)
    with tempfile.TemporaryDirectory(prefix="xiaoyi-weekly-") as temp_dir:
        ...
        return runner_main(["--config", str(generated_config), *forwarded])
